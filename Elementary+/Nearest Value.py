def nearest_value(values, one):
    ...
    smallest_difference = values_list[0] - one
    smallest_difference_index = 0
    
    for i in range(1, len(values_list)):
        if values_list[i] - one < abs(smallest_difference):
            smallest_difference = values_list[i] - one
            smallest_difference_index = i
        
    return values_list[smallest_difference_index]
